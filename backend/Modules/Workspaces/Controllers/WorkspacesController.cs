...
public class WorkspacesController(IWorkspaceService workspaceService) : ControllerBase
{
    ...
    {
        var workspace = await workspaceService.CreateAsync(CurrentUserId, dto, ct);
    ...
    {
        var result = await workspaceService.GetUserWorkspacesAsync(CurrentUserId, page, pageSize, ct);
        return Ok(result);
    }
    ...
    {
        var workspace = await workspaceService.UpdateAsync(slug, CurrentUserId, dto, ct);
        return Ok(workspace);
    }
    ...
    {
        await workspaceService.DeleteAsync(slug, CurrentUserId, ct);
        return NoContent();
    }
    ...
    {
        await workspaceService.RemoveMemberAsync(slug, userId, CurrentUserId, ct);
        return NoContent();
    }
    ...
    {
        var member = await workspaceService.UpdateMemberRoleAsync(slug, userId, dto, CurrentUserId, ct);
        return Ok(member);
    }
    ...
    {
        var invitation = await workspaceService.InviteMemberAsync(slug, dto, CurrentUserId, ct);
    ...
    {
        await workspaceService.AcceptInvitationAsync(token, CurrentUserId, ct);
        return NoContent();
    }
    ...
    {
        await workspaceService.RevokeInvitationAsync(id, CurrentUserId, ct);
        return NoContent();
    }
}
